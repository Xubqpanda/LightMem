...
class OpenaiManager:
    ...
    def meta_text_extract(
        ...
        Args:
            ...
            entry_type: Optional label to stamp onto each cleaned result entry
        ...
        def process_segment_wrapper(args):
            ...
            try:
                ...
                if entry_type:
                    def add_entry_type(payload):
                        if isinstance(payload, dict):
                            if "entry_type" in payload:
                                return payload
                            new_payload = dict(payload)
                            new_payload["entry_type"] = entry_type
                            return new_payload
                        if isinstance(payload, list):
                            return [add_entry_type(item) for item in payload]
                        return payload

                    metadata_facts = add_entry_type(metadata_facts)
