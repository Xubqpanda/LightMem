"""Retrieval helper classes extracted from run_unified_retrieval.py

This module contains reusable retrieval classes used by experiments:
- LLMModel
- QdrantEntryLoader
- TermExtractor
- VectorRetriever
...
Keeping this logic in a separate module makes the experiment runner
...
# Try to import spaCy; caller may rely on SPACY availability
try:
    import spacy
...
# Qdrant imports (may be absent in some environments)
try:
...
class LLMModel:
    """LLM wrapper used by the experiments"""
...
class QdrantEntryLoader:
    """Load entries from Qdrant or fallback to reading the on-disk sqlite storage."""
...
def format_related_memories(related: List[Dict[str, Any]]) -> str:
    ...
    for item in related:
        ...
        formatted_date = dt.strftime("%d %B %Y")  # 例如：14 September 2022
        formatted = f"[Memory recorded on: {formatted_date}, {weekday}]\n{memory}"
    
        out.append(formatted.strip())
    
    return "\n\n".join(out)  # 使用双换行分隔不同的memories
