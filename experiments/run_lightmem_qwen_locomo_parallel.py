...
QDRANT_PRE_UPDATE_DIR='./qdrant_data_locomo_gpt-4o-mini_text-embedding-3-small_512_0_8_pre_update_prompt_with_interaction_collect_token_cunsumption'
QDRANT_POST_UPDATE_DIR='./qdrant_data_locomo_gpt-4o-mini_text-embedding-3-small_512_0_8_post_update_prompt_with_interaction_collect_token_cunsumption'
...
def process_single_sample(sample):
    ...
    try:
        ...
        logger.info(f"\n{'─'*70}")
        logger.info("Step 1: Building memory (add_memory phase)")
        logger.info(f"{'─'*70}")
        ...
        logger.info(f"✓ Add_memory phase completed: {after_add_count} entries in {add_memory_duration:.2f}s")
        
        logger.info(f"\n{'─'*70}")
        logger.info("Step 2: Backing up pre-update state")
        logger.info(f"{'─'*70}")
        ...
        if os.path.exists(backup_dir):
            logger.info(f"  Removing existing backup directory...")
        ...
        logger.info(f"\n{'─'*70}")
        logger.info("Step 3: Performing update (offline_update phase)")
        logger.info(f"{'─'*70}")
        ...
        logger.info(f"\n{'='*70}")
        logger.info(f"STATISTICS FOR: {sample_id}")
        logger.info(f"{'='*70}")
        
        logger.info(f"\n【Directory Information】")
        logger.info(f"  Pre-update (backup): {QDRANT_PRE_UPDATE_DIR}/{sample_id} ({pre_update_count} entries)")
        logger.info(f"  Post-update (final): {QDRANT_POST_UPDATE_DIR}/{sample_id} ({post_update_count} entries)")
        logger.info(f"  Entry change:        {post_update_count - pre_update_count:+d}")
        
        logger.info(f"\n【Time Statistics】")
        logger.info(f"  Total time:        {case_total_duration:.2f}s (100.0%)")
        logger.info(f"  ├─ Add_memory:     {add_memory_duration:.2f}s ({add_memory_duration/case_total_duration*100:.1f}%)")
        ...
        logger.info(f"  API calls:         {case_add_calls}")
        logger.info(f"  Prompt tokens:     {case_add_prompt:,}")
        logger.info(f"  Completion tokens: {case_add_completion:,}")
        logger.info(f"  Total tokens:      {case_add_tokens:,}")
        
        logger.info(f"\n【Update Token Statistics】")
        logger.info(f"  API calls:         {case_update_calls}")
        logger.info(f"  Prompt tokens:     {case_update_prompt:,}")
        logger.info(f"  Completion tokens: {case_update_completion:,}")
        logger.info(f"  Total tokens:      {case_update_tokens:,}")
        
        logger.info(f"\n【Total Token Usage】")
        logger.info(f"  Total API calls:   {case_add_calls + case_update_calls}")
        logger.info(f"  Total tokens:      {case_add_tokens + case_update_tokens:,}")
        logger.info(f"{'='*70}\n")
...
def main():
    ...
    main_logger.info("=" * 70)
    main_logger.info("PARALLEL MEMORY BUILDING STARTED")
    main_logger.info("=" * 70)
    main_logger.info(f"Max workers: {MAX_WORKERS}")
    main_logger.info(f"Executor type: {'ProcessPool' if USE_PROCESS_POOL else 'ThreadPool'}")
    main_logger.info(f"Working directory (post-update): {QDRANT_POST_UPDATE_DIR}")
    main_logger.info(f"Backup directory (pre-update):   {QDRANT_PRE_UPDATE_DIR}")
    main_logger.info("=" * 70)
    
    data = json.load(open(DATA_PATH, "r"))
    main_logger.info(f"\nLoaded {len(data)} samples from LoCoMo dataset")
    
    main_logger.info("\n" + "=" * 70)
    main_logger.info("Phase 1a: Scanning existing collections...")
    main_logger.info("=" * 70)
    ...
    for sample in data:
        ...
        main_logger.info(f"✗ {sample_id}: NEEDS BUILD ({', '.join(status)})")
        missing.append(sample)
    
    main_logger.info(f"\nScan complete: {len(missing)}/{len(data)} samples need building\n")
    
    if not missing:
        main_logger.info("All samples already completed. Nothing to do.")
        return
    
    main_logger.info("=" * 70)
    main_logger.info(f"Phase 1b: Building {len(missing)} samples in parallel")
    main_logger.info("=" * 70)
    ...
    main_logger.info("\n" + "=" * 70)
    main_logger.info("PARALLEL BUILDING COMPLETED")
    main_logger.info("=" * 70)
    ...
    main_logger.info(f"  Total wall time:    {total_duration:.2f}s ({total_duration/60:.2f} min)")
    ...
    if successful:
        ...
        main_logger.info(f"\n【Performance Metrics】")
        main_logger.info(f"  Avg time per sample:  {avg_duration:.2f}s")
        main_logger.info(f"  Speedup ratio:        {avg_duration * len(successful) / total_duration:.2f}x")
    ...
    main_logger.info(f"\n{'='*70}")
    main_logger.info(f"Pre-update backup:  {QDRANT_PRE_UPDATE_DIR}")
    main_logger.info(f"Post-update final:  {QDRANT_POST_UPDATE_DIR}")
    main_logger.info(f"Logs directory:     {RUN_LOG_DIR}")
    main_logger.info("=" * 70)
