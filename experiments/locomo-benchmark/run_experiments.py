...
def main():
    ...
    elif args.technique_type == "lightmem":
        from src.lightmem.add import LightMemADD
        from src.lightmem.search import LightMemSearch
        if args.method == "add":
            lightmem_manager = LightMemADD(data_path="dataset/locomo10.json")
            lightmem_manager.process_all_conversations()
        elif args.method == "search":
            lightmem_manager = LightMemSearch()
            lightmem_manager.process_data_file("dataset/locomo10.json")
    else:
